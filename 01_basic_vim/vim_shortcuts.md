...
Full command reference for the basic Vim markdown setup

## Setup Instructions

### Linux

1.  Install Vim, Pandoc, and a PDF engine:

        sudo apt install vim pandoc wkhtmltopdf

    (Debian/Ubuntu; use your distro's package manager equivalent otherwise, e.g. `dnf`, `pacman`.)
...
2.  Run the bundled setup script from inside this folder — it checks the dependencies above and symlinks `vimrc` to `~/.vimrc` (backing up any existing one first, with confirmation):
    ...
    Or do it by hand instead:
        ...
        ln -s /home/ed/MEGA/app/vim_for_markdown/01_basic_vim/vimrc ~/.vimrc

    Or, if you already have a `~/.vimrc` and don't want to replace it, add this line to the end of it instead:

        source /home/ed/MEGA/app/vim_for_markdown/01_basic_vim/vimrc

3.  Open a markdown file to confirm it works:

        vim notes.md

    Spell-check and markdown highlighting should be active automatically. Try `<leader>p` to preview in browser and `<leader>e` to export to PDF (default leader is `\`).
...
2.  Install Pandoc and a PDF engine:

        winget install --id JohnMacFarlane.Pandoc
        winget install --id wkhtmltopdf.wkhtmltopdf

    Ensure both are added to your `PATH` (the installers usually do this automatically; if `pandoc --version` in a new terminal fails, add their install folders to PATH manually).
...
3.  Find/create your Vim config file. On Windows this is usually:
    ...
    (often `C:\Users\<you>\_vimrc`). Add this line to it, pointing at wherever you've placed this project's `vimrc` file (adjust the path, using forward slashes even on Windows):
        ...
        source C:/path/to/vim_for_markdown/01_basic_vim/vimrc
...
Vim Shortcuts Reference — part of the Basic Vim Markdown Setup project.
