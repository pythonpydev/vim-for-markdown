#!/usr/bin/env bash
# Sets up the enhanced Vim markdown config by symlinking it to ~/.vimrc.
...
echo "== Enhanced Vim Markdown Setup =="
...
if ! command -v fzf >/dev/null 2>&1; then
  echo
  echo "Note: fzf not found — <leader>ff (fuzzy find) and <leader>sa (fuzzy"
  echo "save-as) won't work until it's installed. Everything else will."
  echo "Install it with, e.g. on Debian/Ubuntu:"
...
echo "  vim notes.md"
echo "Leader is \\  —  \\p previews in browser, \\e exports to PDF, \\s toggles"
echo "spell-check, \\ff fuzzy-finds a file, \\sa fuzzy-browses a save-as location."
