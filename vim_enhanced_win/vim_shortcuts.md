...
Full command reference for the enhanced Vim markdown setup — Windows port
...
2.  Install Pandoc, a PDF engine, and fzf:
    ...
    Ensure all three are added to your `PATH` (the installers usually do this automatically; if `pandoc --version` in a new terminal fails, add their install folders to PATH manually, then open a new terminal).

    The winget `fzf` package only gives you `fzf.exe` — it does **not** include fzf's bundled Vim plugin (`plugin/fzf.vim`), which only ships in the git repo. Get that separately:

        git clone --depth 1 https://github.com/junegunn/fzf.git $HOME\.fzf

    (`vimrc`'s runtimepath detection already looks for it there.)

3.  Run the bundled setup script from inside this folder — it checks the dependencies above and points your Vim config at this project's `vimrc`:

        .\setup.ps1

    (If PowerShell blocks the script, run `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass` first, or right-click → Run with PowerShell.)

    Or do it by hand instead — find/create your Vim config file (usually `$HOME\_vimrc`, i.e. `C:\Users\<you>\_vimrc`) and add this line to it, pointing at wherever you've placed this project's `vimrc` file (use forward slashes, even on Windows):

        source C:/path/to/vim_for_markdown/vim_enhanced_win/vimrc
...
This is a straight port of the [`03_vim_enhanced`](../03_vim_enhanced/README.md) config for native Windows Vim (not WSL or Git-Bash Vim). Everything below behaves identically to the Linux version except:

- **fzf plugin discovery** — only checks `~/.fzf` (populated by a git clone, not a package manager; see step 2 above).
- **`<leader>sa`'s folder browser** — uses `dir /b /s /ad` (Windows) instead of `find . -type d` (Unix), since Windows has no `find`.
- **`<leader>p` preview** — writes to `%TEMP%` instead of `/tmp`, and opens the result with `start` instead of `xdg-open`.
- **Theme persistence** — saved under `%LOCALAPPDATA%\vim_enhanced_win\theme` instead of `~/.local/state/...`.
- **No `<leader>r` / `:RefreshTerminal`** — that command worked around a redraw glitch specific to some Linux terminal emulators (gnome-terminal/VTE); it doesn't apply on Windows Terminal/cmd, so it isn't ported.
- **`setup.ps1` instead of `setup.sh`** — appends a `source` line to `$HOME\_vimrc` rather than symlinking, since symlinks need admin rights or Developer Mode on Windows.
...
Vim Shortcuts Reference — part of the Enhanced Vim Markdown Setup project (Windows port).
